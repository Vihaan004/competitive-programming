...
- Supports queries such as "largest value $\le X$"
...
Key idea: instead of checking all pairs or ranges explicitly, maintain two pointers that move based on the current state. This reduces complexity from $O(n^2)$ to $O(n)$ in many cases.
...
Canonical example: given an array $a$ and queries of the form $(l, r)$, output $a_l + a_{l+1} + \dots + a_r$ for each query.

Solution:

- Precompute prefix array: $p[i] = a_1 + a_2 + \dots + a_i$ (with $p[0] = 0$).
- Answer query $(l, r)$ with: $p[r] - p[l-1]$.
...
Canonical example: given an empty array and updates of the form $(l, r, x)$ meaning "add $x$ to $a_l, a_{l+1}, \dots, a_r$", output the array after all updates.

Solution:

- Use difference array $d$ where: $d[i] = a[i] - a[i-1]$.
- For update $(l, r, x)$: add $x$ to $d[l]$, subtract $x$ from $d[r+1]$.
- Reconstruct final array: $a[i] = d[1] + d[2] + \dots + d[i]$.
